...
test('getUnit', () => {
	// no amount given
	expect( q.getUnit('sticks of butter') ).toBe('sticks')
	// amount given
	expect( q.getUnit('2 sticks of butter') ).toBe('sticks')
	// no unit given
...
test('getUnits', () => {
	// 1 unit
	expect(new q.Quantity({unitToAmount: {'cups': 0.5}}).getUnits() ).toStrictEqual(['cups'])
})

test('getCombinedAmountUnitString', () => {
	// 1 unit
	expect(new q.Quantity({unitToAmount: {'cups': 0.5}}).getCombinedAmountUnitString() ).toBe('0.5 cups')
	// 2 units
	expect(new q.Quantity({unitToAmount: {'cups': 0.5, 'tsps': 1}}).getCombinedAmountUnitString() ).toBe('0.5 cups, 1 tsp')
})
...
test('constructQuantityFromObjects', () => {
	...
	expect(quantity.unitToAmount['cups']).toBe(1)
	expect(quantity.unitToAmount['liters']).toBe(3)
})
